...
from typing import Dict as PyDict, Tuple, Optional, Union, Literal
...
def solve_eta_ijk(P: SparseMatrix, N: int, eta_ij: np.ndarray, method: SolverMethod = 'direct') -> np.ndarray:
    ...
    rows, cols, data, rhs = _assemble_eta_ijk_matrix(N, P_dense, triplets_array, triplet_lookup, eta_ij_flat)
...
def solve_eta_system(
    ...
    if check_connectivity:
        ...
        if not nx.is_connected(G):
            raise ValueError(f'Graph must be connected. Found {nx.number_connected_components(G)} components.')
...
def compute_c_values(network: Union[np.ndarray, nx.Graph], k_steps: int = 1, solver_method: SolverMethod = 'gmres') -> PyDict[str, float]:
    if isinstance(network, nx.Graph):
        adj_matrix = nx.to_numpy_array(network)
    else:
        adj_matrix = network

    result = solve_eta_system(adj_matrix, k_steps=k_steps, solver_method=solver_method, verbose=False)
